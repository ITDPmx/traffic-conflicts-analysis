  ...
  return (
    <div className="mx-auto flex w-full items-center">
      <div className="relative mx-auto mt-10 overflow-x-auto shadow-md sm:rounded-lg">
        ...
        {videoIds.length === 0 && (
          <div className="p-4 text-center text-gray-500 ">No hay videos</div>
        )}
        {videoIds.length === 0 &&
  ...
  return (
    <tr className="border-b odd:bg-white even:bg-gray-50 ">
      ...
      <td className="px-6 py-4 underline">
        <Link href={`/api/aws/s3?asset=original_video&id=${data?.id}`}>
        ...
        {(data?.progress ?? 0) < 100 ? (
          ...
          <div className="underline">
            <p>
              <Link
                target="_blank"
                href={`/api/aws/s3?asset=processed_video&id=${data?.id}`}
              >
                Video Procesado
              </Link>
            </p>
            <p>
              <Link
                target="_blank"
                href={`/api/aws/s3?asset=summary&id=${data?.id}`}
              >
                Resumen
              </Link>
            </p>
            <p>
              <Link
                target="_blank"
                href={`/api/aws/s3?asset=full_data&id=${data?.id}`}
              >
                Todos los datos
              </Link>
            </p>
